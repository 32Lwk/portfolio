...
[medicine-recommend-system](https://github.com/32Lwk/medicine-recommend-system) のREADMEでは、非破壊的変換・重症度タグ・escalation_score（閾値4.0で受診勧奨）・Aho-CorasickによるO(n)処理・症状エネルギー保存則による重み正規化が言及されています。
...
- **重症度の閾値**: 強調語から算出した `escalation_score` が**4.0以上**の場合は受診勧奨に繋がるよう設計しています。複数強調語は重み付き加算で判定しています。
- **症状過多の抑制**: 方言を複数症状候補に展開する際、**総症状エネルギー保存則**で重みを正規化し、方言展開による「症状が多すぎる」判定のバイアスを抑えています。
- **診断名のみ判定の例外**: 感情・状態の否定語（あかん、つらい、やばいなど）を含む場合は「診断名のみ」としない例外ルールを入れ、症状として扱うようにしました。
- **打ち身・打撲など**: 「にえる」などから打ち身・打撲・あざ・炎症・内出血などの外傷関連症状を `comprehensive_symptom_list` に追加し、方言からの症状抽出を強化しています。
