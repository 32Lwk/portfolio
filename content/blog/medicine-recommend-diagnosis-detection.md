...
[medicine-recommend-system](https://github.com/32Lwk/medicine-recommend-system) のREADMEでは、診断名検出は**ステップ1.7**（心臓緊急チェックの後、不眠関連キーワードチェックの前）で実行されます。これにより「不眠症」などの診断名がカウンセリングフローに流れず、医師受診を勧告できます。症状表現（「不眠」「眠れない」）は従来どおりカウンセリングで対応します。診断名が検出された場合は通常の医薬品推奨フローをスキップして**早期リターン**し、ChatGPT API を呼ばないためAPIコストを削減しています。
...
- **文脈チェック範囲**: 診断名の**前後50文字**をチェックし、既往歴・持病として言及された場合は検出から除外しています。時間表現（過去・以前・昔）や他人・家族・ペット、治癒表現、逆接表現（ですが・がありますが）なども除外パターンに含め、誤検出を減らしました。
- **不眠症と不眠の区別**: 「不眠症」は診断名として医師受診を勧告し、「不眠」「眠れない」は症状としてカウンセリングフローで対応するよう、不眠関連キーワードから「不眠症」を外し診断名検出側で扱うようにしました。
- **早期リターン**: 診断名検出時は症状分析・医薬品推奨のAPIを呼ばず早期リターンすることで、APIコストを削減しています。
