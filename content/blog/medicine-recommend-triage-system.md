...
- **信頼度閾値**: LLMの信頼度が0.7未満の場合はキーワードマッチングにフォールバックする運用にし、誤分類を抑えています。
- **心臓まわり**: 「心臓が痛い」と「心が痛い」の区別のため、比喩的表現の検出や会話履歴の活用、動的閾値（恋愛文脈0.7・比喩0.8など）を導入しました。
- **Otherの2段階分類**: 店舗案内・不適切要求など20種のサブカテゴリに振り分けることで、医薬品推奨フローに不要な入力を早く処理し、パフォーマンスと一貫性を確保しています。
- **キャッシュ**: 同一入力に対するトリアージ結果をキャッシュし、応答時間を短縮しています。
