...
[medicine-recommend-system](https://github.com/32Lwk/medicine-recommend-system) のREADMEでは、店舗案内はOtherカテゴリの2段階トリアージの一部として、在庫・施設・免税・営業時間・支払い・駐車場・遺失物など複数サブカテゴリに対応しています。商品リストは **`data/store_products.json`** で、サブカテゴリ202・商品名2,362件・ブランド名880件を管理しています。
...
- **処理順序**: 店舗関連の質問は医薬品推奨フローより**先に**処理する早期リターンにして、症状キーワードがない場合は店舗案内を優先しています。
- **在庫確認キーワード**: 「どこ」「場所は」を追加し、「歯ブラシはどこ？」「化粧水の場所は？」のような表現にも対応しました。商品名が検出されかつこれらのキーワードがある場合は在庫確認として優先しています。
- **症状との区別**: 「うんこしたい」は症状キーワードがなければ店舗案内（トイレ案内）として扱い、「出ない」「便秘」「痛い」などが含まれる場合は医薬品推奨を優先するようにしています。
- **信頼度**: LLMの信頼度が0.7未満の場合はキーワードマッチングにフォールバックし、店舗案内の誤検出を抑えています。
